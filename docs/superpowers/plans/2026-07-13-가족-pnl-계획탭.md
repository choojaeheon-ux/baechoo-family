...
> ⚠️ 클라우드 모드라면 `plan_items` 테이블이 아직 없어서 select가 실패하고 `planItems`가 빈 배열이 된다(잔액 0). **Task 6에서 DDL을 적용하기 전까지는 정상이다.** 지금 화면을 검증하려면 `.env.local`을 `.env.local.bak`으로 임시 rename해 **localStorage 모드**로 띄운다(`SEED_PLAN_ITEMS`가 그대로 들어간다). 확인 후 반드시 되돌린다.
